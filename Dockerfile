...
RUN lynx -listonly -dump https://jenkins.mono-project.com/view/Xamarin.Android/job/xamarin-android-linux/lastSuccessfulBuild/Azure/ | grep "https://.*/Azure/processDownloadRequest/xamarin-android/xamarin.android-oss_v" | tail -1 | sed 's/.*. //' > link.txt 

RUN curl -L $(cat link.txt) \
        -o xamarin.tar.bz2
RUN bzip2 -cd xamarin.tar.bz2 | tar -xvf -
RUN mkdir -p /android/xamarin
RUN ls
RUN mv xamarin.android* /android/xamarin && \
        rm xamarin.tar.bz2
